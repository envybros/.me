...
무언가에 대한 불변 참조가 있다면, 동시에 가변 참조를 취할 수 없다는 참조 규칙을 기억해보자. clear는 String을 잘라내야 하므로 가변 참조가 필요하다. clear 호출 후에 println!이 word의 참조를 사용하기 때문에, 그 시점에서 불변 참조가 여전히 활성화되어 있어야 한다. Rust는 clear의 가변 참조와 word의 불변 참조가 동시에 존재하는 것을 허용하지 않아, 컴파일이 실패한다. Rust는 우리의 API를 사용하기 쉽게 만드는 것뿐만 아니라, 컴파일 시간에 전체 에러 범주를 제거했다!
