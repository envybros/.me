...
이전 예제와 마찬가지로, 이 문제는 각 루프 반복에서 수행되는 작업이 일정하게 상각되기 때문에 $$O(n)$$ 시간으로 실행되며, 여기서 $$n$$은 `s`의 길이다. 또한 몇 개의 정수 변수만 사용되므로 이 알고리즘은 $$O(1)$$ 공간을 사용한다.
...
방금 배운 속성을 설명의 예시를 통해 살펴보자. 인덱스 `2`에 도달하면 곱이 너무 커지므로 가장 왼쪽에 있는 요소 `10`을 제거해야 한다. 이제 윈도우은 유효하며 길이는 `2`이다. 즉, 여기서 끝나는 `2`개의 유효한 부분 배열이 있다(`[2]` 및 `[5, 2]`).

이전 예제에서 동안 루프 이후에 답(가장 긴 길이)을 업데이트했음을 기억하자.
윈도우이 유효해야 한다. 여기에서는 대신 윈도우의 현재 크기를 답에 추가할 수 있다. 윈도우가 유효한지 결정하는 제약 조건은 곱이 `k`보다 작아야 한다는 것이다.

또한 `k <= 1`이면 유효한 윈도우을 가질 수 없으므로 즉시 `0`을 반환하면 된다.
...
다시 말하지만, 각 루프 반복에서 수행되는 작업은 일정하게 상각되므로 이 알고리즘의 런타임은 $$O(n)$$이며, 여기서 $$n$$은 `nums`의 길이와 $$O(1)$$의 공간이다.

---

## **고정 윈도우 크기**

위에서 살펴본 예제에서는 윈도우 크기가 동적이었다. 윈도우를 일정한 제약 조건 내에 유지하면서 최대한 오른쪽으로 확장하려고 시도했고, 제약 조건을 위반하면 왼쪽에서 요소를 제거했다. 때때로 문제가 **고정된** 길이 `k`를 지정하는 경우가 있다.

이러한 문제는 인접한 두 윈도우 사이의 차이가 두 요소에 불과하기 때문에 쉽다(오른쪽에 요소 하나를 추가하고 왼쪽에 요소 하나를 제거하여 길이를 유지한다).

첫 번째 윈도우를 만드는 것으로 시작한다(인덱스 `0`에서 `k-1`까지). 크기가 `k`인 윈도우가 만들어지면 인덱스 `i`에 요소를 추가하면 인덱스 `i - k`에서 요소를 제거해야 한다. 예를 들어, `k = 2`이고 현재 인덱스 `[0, 1]`에 요소가 있으므로 이제 `2`: `[0, 1, 2]`를 추가한다. 윈도우 크기를 `k = 2`로 유지하려면 `2 - k = 0: [1, 2]`를 제거해야 한다.
...
> 예제 4. 정수 배열 `num`와 정수 `k`가 주어졌을 때, 길이가 `k`인 가장 큰 합을 가진 부분 배열의 합을 구하라.
{: .prompt-general }

앞서 언급했듯이 길이 `k`의 윈도우를 만든 다음 배열을 따라 슬라이드하면 된다. 한 번에 한 요소씩 추가하고 제거하여 윈도우가 `k` 크기를 유지하도록 한다. `i`에 값을 추가하는 경우 `i - k`에서 값을 제거해야 한다.

첫 번째 윈도우를 만든 후에는 첫 번째 윈도우의 합계를 고려하기 위해 `curr`에 대한 답을 초기화한다.
...
루프 반복의 총합은 $$n$$과 같으며, 여기서 $$n$$은 `nums`의 길이이고 각 반복에서 수행되는 작업은 일정하므로 이 알고리즘의 시간 복잡도는 $$O(1)$$의 공간을 사용하는 $$O(n)$$이다.

---

## **마무리**

슬라이딩 윈도우는 매우 일반적이고 다양한 패턴으로 활용됩니다. 많은 슬라이딩 윈도우 문제에서 해시맵을 사용해야 하기 때문에 여기서는 표면적인 부분만 다루었다. 해시맵에 대해 배운 다음에는 슬라이딩 윈도우 문제를 몇 가지 더 살펴보겠다. 그 동안 다가오는 연습 문제를 풀면서 지식을 테스트해 보자.
